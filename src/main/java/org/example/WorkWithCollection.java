...
public class WorkWithCollection {
    ...
    private static void performTests(List<String> list) {
        ...
        long addFirstTime = testAddFirst(list);
        System.out.printf("%-20s %-20d %-15d%n", "addFirst", NUM_OPERATIONS, addFirstTime);

        long addMiddleTime = testAddMiddle(list);
        ...
        long deleteTime = testDelete(list);
        System.out.printf("%-20s %-20d %-15d%n", "delete", NUM_OPERATIONS, deleteTime);

        long deleteFirstTime = testDeleteFirst(list);
     ...
     */
    public static long testAdd(List<String> list) {
     ...
     */
    public static long testAddFirst(List<String> list) {
        long startTime = System.nanoTime();
        for (int i = 0; i < NUM_OPERATIONS; i++) {
            list.add(0, "Element " + i); // Вставка в начало
        }
        return System.nanoTime() - startTime;
    }
     ...
     */
    public static long testAddMiddle(List<String> list) {
        long startTime = System.nanoTime();
        for (int i = 0; i < NUM_OPERATIONS; i++) {
            list.add(list.size() / 2, "Element " + i); // Вставка в середину
        }
        return System.nanoTime() - startTime;
    }
     ...
     */
    public static long testGet(List<String> list) {
        long startTime = System.nanoTime();
        for (int i = 0; i < NUM_OPERATIONS; i++) {
     ...
     */
    public static long testDelete(List<String> list) {
        long startTime = System.nanoTime();
        for (int i = 0; i < NUM_OPERATIONS; i++) {
            if (!list.isEmpty()) {
     ...
     */
    public static long testDeleteFirst(List<String> list) {
        long startTime = System.nanoTime();
        for (int i = 0; i < NUM_OPERATIONS; i++) {
            if (!list.isEmpty()) {
